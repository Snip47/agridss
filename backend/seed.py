...
def seed():
    ...
    try:
        ...
        # ── Crops ──
        if db.query(Crop).count() < 10:
            from data.crops_db import get_all_crops
            crops = get_all_crops()
            for c in crops:
                if not db.query(Crop).filter(Crop.name==c["name"]).first():
                    db.add(Crop(
        ...
        # ── Livestock ──
        if db.query(Animal).count() < 5:
            from data.livestock_db import get_all_livestock
            animals = get_all_livestock()
            for a in animals:
                if not db.query(Animal).filter(Animal.name==a["name"]).first():
                    db.add(Animal(
