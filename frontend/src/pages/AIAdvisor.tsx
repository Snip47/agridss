...
import { Bot, Send, Leaf, Loader2, Zap, Camera, X, User } from 'lucide-react'
  ...
  return (
    <div className={`flex gap-3 ${isUser?'flex-row-reverse':''}`}>
      ...
      <div className="max-w-2xl space-y-2">
        {msg.image && (
          <div className="rounded-2xl overflow-hidden" style={{ border:'1px solid rgba(255,255,255,0.2)', maxWidth:'280px' }}>
            <img src={msg.image} alt="uploaded" className="w-full h-44 object-cover"/>
          </div>
        )}
        {msg.content && (
...
export default function AIAdvisor() {
  const [messages, setMessages] = useState<Message[]>([
    { role:'assistant', content:"Habari! I am your AgriDSS Farm Advisor 🌾\n\nAsk me anything about farming in Kenya — crops, livestock, diseases, market prices — or upload a photo of your sick crop or animal for instant AI diagnosis.\n\nNinaweza kukusaidia kwa Kiingereza au Kiswahili." }
  ])
  ...
  const send = async (text?: string) => {
    ...
    const userMsg: Message = { role:'user', content:msg||'Please analyze this image and diagnose any problems.', image:capturedImage||undefined }
    const history = messages.slice(1).map(m=>({ role:m.role, content:m.content }))
    setMessages(prev=>[...prev, userMsg]); setLoading(true)
    try {
      let reply = ''
      if (capturedImage) {
        const r = await api.post('/ai/analyze-image', { image:capturedImage, message:msg||'Analyze this crop/animal image. Identify disease, pest damage or health issue. Give diagnosis, severity, treatment with Kenya product names, and prevention.', provider })
        reply = r.data.reply
      } else {
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)] slide-up">
      ...
      <div className="mb-3 flex-shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/40 text-xs">Kenya · English & Swahili · Photo diagnosis</p>
          </div>
        </div>
      ...
      {selectedImage && (
        <div className="flex-shrink-0 mb-2 flex items-center gap-3 p-3 rounded-xl"
          ...
          <div className="relative flex-shrink-0">
            ...
            <button onClick={()=>setSelectedImage(null)} className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full flex items-center justify-center text-white" style={{ background:'rgba(239,68,68,0.9)' }}>
              <X className="w-3 h-3"/>
            </button>
          </div>
          <p className="text-sm text-green-300 font-medium">Photo ready — press send or describe the problem</p>
        </div>
      )}
      ...
      <div className="flex-shrink-0 flex gap-2 p-2 rounded-2xl"
        ...
        <button onClick={()=>fileRef.current?.click()}
          ...
          style={{ background:'rgba(34,197,94,0.2)', border:'1px solid rgba(34,197,94,0.3)' }} title="Upload photo for diagnosis">
          <Camera className="w-4 h-4 text-green-400"/>
        </button>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&send()}
          placeholder="Ask about crops, livestock, diseases... or upload a photo"
          disabled={loading}
