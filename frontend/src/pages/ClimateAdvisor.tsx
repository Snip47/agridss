...
export default function ClimateAdvisor() {
  ...
  useEffect(() => { 
    api.get('/location/counties').then(r=>setCounties(r.data))
    setBgImage(getBackgroundImage('climate'))
  }, [])
  ...
  return (
    <div className="relative min-h-screen" style={{backgroundImage: `url('${bgImage}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed'}}>
      {/* Overlay */}
      <div className="absolute inset-0 bg-black/35 backdrop-blur-sm"></div>
      
      <div className="relative z-10">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white drop-shadow-lg flex items-center gap-2"><MapPin className="w-6 h-6"/> Climate & Location Advisor</h1>
          <p className="text-white/90 mt-1 text-sm drop-shadow">Select your exact location to get climate analysis, agro-ecological zone info, and specific crop & livestock recommendations.</p>
        </div>

        {/* Location selector */}
        <div className="bg-white/95 rounded-xl p-6 shadow-lg border border-earth-100 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="flex items-end">
              <button onClick={analyze} disabled={!sel.county||!sel.constituency||loading}
                className="w-full bg-leaf-600 hover:bg-leaf-700 text-white font-semibold py-2 rounded-lg text-sm transition-colors disabled:opacity-40 flex items-center justify-center gap-2 shadow-lg">
        ...
        {climate && (
          <div className="space-y-5">
            {/* Climate summary */}
            <div className="bg-gradient-to-r from-sky-700/95 to-sky-600/95 rounded-xl p-6 text-white shadow-lg backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="bg-white/95 rounded-xl p-5 shadow-lg border border-earth-100">
              ...
              {climate.recommended_crops.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {climate.recommended_crops.map(crop=>(
                    <div key={crop.id} className="border border-earth-100 rounded-lg p-3 hover:border-leaf-300 transition-colors bg-white">
            ...
            <div className="bg-white/95 rounded-xl p-5 shadow-lg border border-earth-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {climate.recommended_animals.map(a=>(
                  <div key={a.id} className="border border-earth-100 rounded-lg p-3 hover:border-earth-300 transition-colors bg-white">
        ...
        {!climate && !loading && (
          <div className="bg-white/95 rounded-xl border border-earth-100 p-16 text-center shadow-lg">
            <div className="text-6xl mb-4">🗺️</div>
            <h3 className="font-bold text-earth-700 text-lg">Select Your Location Above</h3>
            <p className="text-earth-400 text-sm mt-2 max-w-sm mx-auto">Choose your county and constituency to see your agro-ecological zone, climate data, and best crops & livestock for your specific region.</p>
          </div>
        )}
      </div>
    </div>
