...
interface Crop { id:number; name:string; category:string; subcategory:string; varieties:Variety[]; suitable_aez:string[]; rainfall_min_mm:number; rainfall_max_mm:number; altitude_min_m:number; altitude_max_m:number; maturity_days:number; water_requirement:string; description:string; care_tips:string; best_counties?:string[]; diseases?:string[]; soil_types?:string[]; planting_months?:string[]; expected_yield:string; market_price_ksh?:string }
...
export default function CropAdvisor() {
  ...
  useEffect(()=>{ 
    fetch()
    setBgImage(getBackgroundImage('crops'))
  }, [filters.category])
  ...
  return (
    <div className="relative min-h-screen" style={{backgroundImage: `url('${bgImage}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed'}}>
      ...
      <div className="relative z-10">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white drop-shadow-lg flex items-center gap-2"><Sprout className="w-6 h-6"/> Crop Advisor</h1>
          <p className="text-white/90 mt-1 text-sm drop-shadow">{crops.length > 0 ? `${crops.length} crops` : 'All crops'} with specific varieties, planting guides, and market info.</p>
        </div>
        ...
        {loading ? (
          <div className="text-center py-16 text-white drop-shadow">Loading crops...</div>
        ) : crops.length === 0 ? (
          <div className="text-center py-16 text-white drop-shadow bg-white/10 rounded-xl border border-white/20">No crops found for these filters.</div>
        ) : (
          <div className="space-y-2">
            {crops.map(crop=>(
              <div key={crop.id} className="bg-white/95 rounded-xl shadow-lg border border-earth-100 overflow-hidden">
                ...
                {expanded===crop.id && (
                  <div className="px-5 pb-5 border-t border-earth-100 pt-4 bg-white/98">
                    {/* Varieties */}
