...
export default function CropAdvisor() {
  ...
  const deleteCrop = async (id:number) => {
    ...
    setDeleting(id); await api.delete(`/crops/${id}`)
    setCrops(c=>c.filter(x=>x.id!==id))
    if (selected?.id===id) setSelected(null); setDeleting(null)
  }
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input placeholder="Search crops..." value={filters.search}
              ...
              className="w-full pl-9 rounded-xl px-3 py-2 text-sm text-white placeholder-white/30 outline-none"
              style={{ background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.18)' }}/>
          </div>
          <button onClick={fetchCrops} className="px-4 py-2 rounded-xl text-sm font-bold text-white"
            style={{ background:'rgba(34,197,94,0.7)' }}>Search</button>
        </div>
      </G>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-1 space-y-2 max-h-[75vh] overflow-y-auto pr-1">
          {loading && <div className="text-center py-8 text-white/40 text-sm">Loading...</div>}
          {!loading && crops.length===0 && <G className="py-8 text-center text-white/40 text-sm">No crops found.</G>}
          {crops.map(crop=>(
            ...
            <G className="overflow-hidden">
              <div className="relative p-8 flex items-center gap-5"
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    <span className="text-xs text-white/40"><Clock className="w-3 h-3 inline mr-0.5"/>{selected.maturity_days} days</span>
                  </div>
                </div>
              ...
              <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto scrollbar-thin">
                ...
                {selected.suitable_aez?.length>0 && (
                  <div><h4 className="text-xs font-black text-white/50 uppercase tracking-wider mb-2">🌍 Where to Grow</h4>
                    <div className="flex flex-wrap gap-1">{selected.suitable_aez.map(z=><Badge key={z} color="rgba(96,165,250,0.2)" text="#93c5fd">{z}</Badge>)}</div>
                  </div>
                )}
                {selected.best_counties?.length>0 && (
                  <div><h4 className="text-xs font-black text-white/50 uppercase tracking-wider mb-2">📍 Best Counties</h4>
                    <div className="flex flex-wrap gap-1">{selected.best_counties.map(c=><Badge key={c} color="rgba(167,139,250,0.2)" text="#c4b5fd">{c}</Badge>)}</div>
                  </div>
                )}
                {selected.soil_types?.length>0 && (
                  <div><h4 className="text-xs font-black text-white/50 uppercase tracking-wider mb-2">🏔️ Soil Types</h4>
                    <div className="flex flex-wrap gap-1">{selected.soil_types.map(s=><Badge key={s} color="rgba(251,191,36,0.2)" text="#fde68a">{s}</Badge>)}</div>
                  </div>
                )}
                {selected.planting_months?.length>0 && (
                  <div><h4 className="text-xs font-black text-white/50 uppercase tracking-wider mb-2">📅 Planting Months</h4>
                    <div className="flex flex-wrap gap-1">{selected.planting_months.map(m=><Badge key={m} color="rgba(34,197,94,0.2)" text="#4ade80">{m}</Badge>)}</div>
                  </div>
                )}
                {selected.varieties?.length>0 && (
                  <div><h4 className="text-xs font-black text-emerald-400 uppercase tracking-wider mb-2">🌱 Varieties</h4>
                ...
                <div><h4 className="text-xs font-black text-white/50 uppercase tracking-wider mb-2">🌿 Care Tips</h4>
                  <p className="text-sm text-white/70 leading-relaxed">{selected.care_tips}</p>
                </div>
                {selected.diseases?.length>0 && (
                  <div><h4 className="text-xs font-black text-red-400 uppercase tracking-wider mb-2">⚠️ Watch Out For</h4>
                    <div className="flex flex-wrap gap-1">{selected.diseases.map(d=><Badge key={d} color="rgba(239,68,68,0.15)" text="#fca5a5">{d}</Badge>)}</div>
                  </div>
                )}
              </div>
            </G>
