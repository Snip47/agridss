...
export default function Dashboard() {
  ...
  useEffect(() => { 
    ...
    setBgImage(getBackgroundImage('dashboard'))
  }, [])
  ...
  return (
    <div className="relative min-h-screen" style={{backgroundImage: `url('${bgImage}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed'}}>
      ...
      <div className="relative z-10">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white drop-shadow-lg">Welcome back, {user?.name?.split(' ')[0]}! 👋</h1>
          <p className="text-white/90 mt-1 text-sm drop-shadow">Kenya Agricultural Decision Support System — AI-powered farming intelligence.</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {cards.map(({ label, val, icon:Icon, bg, text, to }) => (
            <Link key={label} to={to} className="bg-white/95 rounded-xl p-5 shadow-lg border border-earth-100 hover:shadow-xl hover:scale-105 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
          {/* Quick actions */}
          <div className="lg:col-span-2 bg-white/95 rounded-xl p-6 shadow-lg border border-earth-100">
          ...
          {/* Activity */}
          <div className="bg-white/95 rounded-xl p-6 shadow-lg border border-earth-100">
        ...
        {/* User location info */}
        {user?.county && (
          <div className="bg-white/95 rounded-xl p-5 shadow-lg border border-earth-100 mb-5">
        ...
        {/* Kenya banner */}
        <div className="bg-gradient-to-r from-leaf-700/95 to-leaf-600/95 rounded-xl p-6 text-white shadow-lg backdrop-blur-sm">
          <div className="flex items-center gap-4">
            <div className="text-4xl">🇰🇪</div>
            <div>
              <div className="font-bold text-base">Kenya-Focused Agriculture Intelligence</div>
              <div className="text-leaf-100 text-xs mt-1">47 Counties · County → Constituency → Ward location drill-down · 40+ Crop varieties · 14 Livestock species with specific breeds · Free AI Advisor</div>
            </div>
          </div>
        </div>
      </div>
