...
import { getAnimalImage, getCatImage } from '../lib/itemImages'
  ...
  return (
    <div className="slide-up">
      <div className="mb-5">
        ...
        <p className="text-white/45 mt-1 text-sm">{animals.length} livestock with breeds, feeding, housing and market info</p>
      </div>
            ...
            <div key={animal.id} onClick={()=>{ setSelected(animal); setActiveTab('overview') }}
              ...
              <div className="flex items-center gap-3 p-3">
                ...
                <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0">
                  <img
                    src={getCatImage(animal.category)}
                    alt={animal.name}
                    className="w-full h-full object-cover"
                    onError={e=>{(e.target as HTMLImageElement).src='https://images.unsplash.com/photo-1500595046743-cd271d694d30?w=200&q=80'}}
                  />
                </div>
          ...
          {selected ? (
            <G className="overflow-hidden">
              ...
              <div className="relative h-56 overflow-hidden">
                <img
                  src={getCatImage(selected.category)}
                  alt={selected.name}
                  className="w-full h-full object-cover"
                  onError={e=>{(e.target as HTMLImageElement).src='https://images.unsplash.com/photo-1500595046743-cd271d694d30?w=800&q=80'}}
                />
                ...
                <div className="absolute bottom-4 left-5 right-5">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ background:'rgba(251,191,36,0.7)', color:'black' }}>
                      {CAT_EMOJI[selected.category]} {selected.category}
                    </span>
                    <Badge color="rgba(255,255,255,0.2)" text="white">{selected.purpose}</Badge>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 max-h-[45vh] overflow-y-auto scrollbar-thin space-y-4">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          {selected.breeds.map((b,i)=>(
                            <div key={i} className="rounded-xl overflow-hidden" style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)' }}>
                              <div className="flex gap-3 p-3">
                                ...
                                <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0">
                                  <img
                                    ...
                                    alt={b.name}
                                    className="w-full h-full object-cover"
                                    onError={e=>{(e.target as HTMLImageElement).src=getCatImage(selected.category)}}
                                  />
                                </div>
                                <div className="flex-1">
                                  ...
                                  {(b.milk_production||b.weight_kg||b.eggs_year||b.honey_kg_yr) && (
                                    <div className="text-xs font-bold mb-1" style={{ color:'#fbbf24' }}>
                ...
                {activeTab==='vaccines' && (
                  <div className="space-y-2">
                    {selected.vaccination_schedule?.map((v,i)=>(
                    ...
                    {(!selected.vaccination_schedule||selected.vaccination_schedule.length===0) && (
            ...
            <G className="h-full flex items-center justify-center py-24">
              <div className="text-center">
                ...
                <p className="text-white/25 text-sm mt-1">Click any animal to see breeds, feeding guide and market info</p>
              </div>
            </G>
          )}
        </div>
