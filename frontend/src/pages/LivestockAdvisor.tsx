...
const CAT_COLOR:Record<string,string> = { cattle:'rgba(251,191,36,0.15)',goat:'rgba(34,197,94,0.15)',sheep:'rgba(255,255,255,0.12)',poultry:'rgba(249,115,22,0.15)',rabbit:'rgba(236,72,153,0.15)',pig:'rgba(251,146,60,0.15)',fish:'rgba(96,165,250,0.15)',bees:'rgba(234,179,8,0.15)',camel:'rgba(180,83,9,0.15)',donkey:'rgba(107,114,128,0.15)' }
              ...
              <G key={animal.id} className="overflow-hidden hover:border-white/20 transition-all">
                <div className="flex items-center gap-3 px-5 py-4 cursor-pointer" onClick={()=>setExpanded(expanded===animal.id?null:animal.id)}>
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                    style={{ background:CAT_COLOR[animal.category]||'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)' }}>
                                ...
                                <div key={i} className="p-4 rounded-xl"
                                  ...
                                  <div className="flex items-start justify-between gap-2 mb-2">
                                    <div>
                                      ...
                                      <div className="text-xs text-white/40">Origin: {b.origin}</div>
                                    </div>
                      ...
                      {tab==='feeding' && (
                        <div>
                          ...
                          {animal.common_diseases?.length>0 && (
                            <div>
                              <h4 className="text-xs font-black text-red-400 uppercase mb-2">⚠️ Common Diseases</h4>
                          ...
                          {animal.vaccination_schedule?.map((v,i)=>(
                            <div key={i} className="flex items-start justify-between p-3 rounded-xl gap-3"
                              ...
                              <span className="text-xs px-2 py-1 rounded-full font-medium flex-shrink-0"
                                style={{ background:'rgba(96,165,250,0.2)', color:'#93c5fd', border:'1px solid rgba(96,165,250,0.3)' }}>{v.dose}</span>
                            </div>
                          ))}
                          {(!animal.vaccination_schedule||animal.vaccination_schedule.length===0) && (
