...
interface Animal { id:number; name:string; category:string; purpose:string; breeds:Breed[]; suitable_aez:string[]; description:string; feeding_guide:string; housing_requirements:string; vaccination_schedule?:any[]; breeding_info:string; market_info?:string; water_requirement:string; space_required:string; common_diseases?:string[] }
...
export default function LivestockAdvisor() {
  ...
  useEffect(() => { 
    fetch()
    setBgImage(getBackgroundImage('livestock'))
  }, [filters.category, filters.purpose])
  ...
  return (
    <div className="relative min-h-screen" style={{backgroundImage: `url('${bgImage}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed'}}>
      ...
      <div className="relative z-10">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white drop-shadow-lg flex items-center gap-2"><Beef className="w-6 h-6"/> Livestock Advisor</h1>
          <p className="text-white/90 mt-1 text-sm drop-shadow">{animals.length} livestock types with specific breeds, feeding, housing, vaccination and market info.</p>
        </div>

        {/* Filters */}
        <div className="bg-white/95 rounded-xl p-4 shadow-lg border border-earth-100 mb-5">
        ...
        {loading ? (
          <div className="text-center py-16 text-white drop-shadow">Loading livestock...</div>
        ) : (
          <div className="space-y-2">
            {animals.length === 0 && (
              <div className="text-center py-16 text-white drop-shadow bg-white/10 rounded-xl border border-white/20">No livestock found.</div>
            )}
            {animals.map(animal => {
              const tab = activeTab[animal.id] || 'breeds'
              return (
                <div key={animal.id} className="bg-white/95 rounded-xl shadow-lg border border-earth-100 overflow-hidden">
                  ...
                  {expanded === animal.id && (
                    <div className="border-t border-earth-100">
                      {/* Tab bar */}
                      <div className="flex border-b border-earth-100 overflow-x-auto bg-white/98">
                        {[
